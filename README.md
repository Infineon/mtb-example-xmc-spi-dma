...
[Provide feedback on this code example.](https://cypress.co1.qualtrics.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)

## Requirements

- [ModusToolbox&trade;](https://www.infineon.com/modustoolbox) v3.1 (tested with v3.2)
...
The following example clones the "[mtb-example-xmc-spi-dma](https://github.com/Infineon/mtb-example-xmc-spi-dma)" application with the desired name "SPI_DMA" configured for the *KIT_XMC47_RELAX_V1* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_XMC47_RELAX_V1 --app-id mtb-example-xmc-spi-dma --user-app-name SPI_DMA --target-dir "C:/mtb_projects"
   ```
...
5. Observe LED1 blinking which indicates successful data transfer using SPI.

6. To illustrate a transfer error, remove the external connection made in **Step 1** and observe the LED2 glow.

> **Note:** Only KIT_XMC_PLT2GO_XMC4200 does not have two LEDs. The illustration of a transfer error will be the same as in **Step 6** with LED1 constantly glowing.

7. The macro `ENABLE_XMC_DEBUG_PRINT` in *main.c* is disabled by default. When this macro is enabled, code execution logs can be seen in an UART terminal such as TeraTerm, Putty etc.

> **Note:** Code execution logs can be seen for all BSPs except KIT_XMC_PLT2GO_XMC4400. This is because the SPI channel and UART channel that are supported on the kit hardware belong to the same USIC block. Therefore, SPI and UART cannot be enabled at the same time.
...
Document title: *CE232726* – *XMC&trade; MCU: SPI DMA*
 ...
 2.2.0   | Removed the UART debug logs for KIT_XMC_PLT2GO_XMC4400 
<br>
