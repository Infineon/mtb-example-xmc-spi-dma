...
In this example, the DMA is configured to transfer a data array from RAM to SPI buffer.
...
An external jumper wire is required to connect between MOSI(P0_5) and MISO(P0_4).
...
## Operation
1. Externally connect the jumper wire between MOSI() and MISO().
...
6. To illustrate a transfer error, remove the external connection made in step 1 and observe LED2 glow.
...
In this example project, SPI is configured so that a data buffer(src) is transferred through SPI MOSI to the MISO of the same SPI Component. After the transfer, the received data is compared with the transmitted data. If the transfer is successful, the data received is the same as the data transmitted. The LEDs indicate the success or failure of the data transfer. DMA helps in offloading the data transfer from CPU, allowing it to perform other tasks. The role of DMA in this code example is to place the firmware data buffer(src) on the SPI output FIFO and also to read the data buffer(dst) from SPI input FIFO after the transfer. 
...
Document Title: *CE232726* - *XMC MCU: SPI DMA*
